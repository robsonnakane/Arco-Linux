...
# Função para ativar o serviço SSH
function ativar_ssh() {
  sudo systemctl status sshd 2>&1 | grep -q "active (running)"
  if [ $? -eq 0 ]; then
    echo "Serviço SSH já está ativo."
  else
    sudo systemctl start sshd
    sudo systemctl enable sshd 
    sudo ufw allow 22/tcp
    echo "Serviço SSH iniciado e habilitado."
  fi
}

# Lista de pacotes a serem instalados
pacotes=("openssh" "rsync" "fastfetch" "libnotify" "foomatic-db" "flatpak" "jdk-openjdk" "boxes" "thunderbird" "vlc" "audacious")
